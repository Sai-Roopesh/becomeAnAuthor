...
| `DocumentExportService` | `DocumentExportService.ts` | ~1260 lines | Full manuscript export (DOCX, PDF via @react-pdf/renderer, Markdown) with ExportConfigV2 support. Includes section-aware structure controls (section headings, TOC inclusion, per-type page breaks, excluded-section filtering) plus robust text sanitization for PDF stability. |
