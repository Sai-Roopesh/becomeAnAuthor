...
| ----------------- | -------- | ---------------------------------------------- | --------------------------------------------------------------- |
| `useProjectStore` | App-wide | SQLite `app_preferences["ui.project_store"]`   | Active scene, active project path, view mode, panel visibility, sidebar tabs         |
...
| -------------- | ------------------------------ | -------------------------------------------------------------------------------------- |
| **DOCX**       | Frontend `docx` (npm)          | Native Word document with config (font, size, margins, TOC). Preserves @mentions text. |
| **EPUB**       | Rust `epub-builder`            | eBook with chapters, metadata, CSS styling                                             |
| **PDF**        | Frontend `@react-pdf/renderer` | Native React PDF rendering. Preserves @mentions text.                                  |
| **Markdown**   | Rust string assembly           | Plain markdown with scene concatenation. Preserves @mentions text.                     |
...
| **Full Snapshot Package** | Full app state          | `.baa` `full_snapshot` export/import with auto-checkpoint and staged replace restore. Backed by SQL-native `db/payload.db` |
| **Series Package**        | One series + all novels | `.baa` `series_package` clone import with full series codex graph via SQL-native merge                                     |
| **Novel Package**         | One novel               | `.baa` `novel_package` clone import with referenced codex subset via SQL-native merge                                      |
...
- **Validation**: CI runs `release:gate` checks (lint, typecheck, build, cargo test), verifies version match, builds multi-platform binaries, and generates signatures.
