...
import { parseSSEStream } from '@/core/api/streaming-utils';
 ...
 * Providers that support dynamic model fetching: openrouter, google, mistral, kimi
 * Providers with static/default models: openai, azure, fireworks, etc.
 */
export async function fetchModelsForConnection(connection: AIConnection): Promise<string[]> {
    ...
    const dynamicModelProviders: AIProvider[] = ['openrouter', 'google', 'mistral', 'kimi'];
    const supportsDynamicFetch = dynamicModelProviders.includes(connection.provider);

    // If vendor doesn't support dynamic fetching, return default models
    if (!supportsDynamicFetch) {
