...
class SaveCoordinator {
  ...
  async scheduleSave(
    sceneId: string,
    getContent: () => import("@/shared/types/tiptap").TiptapContent,
    wordCount: number,
  ): Promise<void> {
    ...
    const saveOperation = (async () => {
      ...
      try {
        ...
        log.debug(`Got content for scene ${sceneId}`, {
        ...
        if (
          errorMessage.includes("Scene not found") ||
          errorMessage.includes("not found in structure")
        ) {
