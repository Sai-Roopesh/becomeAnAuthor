...
describe("SaveCoordinator Contract", () => {
  ...
  describe("Race Condition Prevention - MUST serialize saves per scene", () => {
    it("MUST execute saves sequentially for the same scene", async () => {
      ...
      const save1 = saveCoordinator.scheduleSave(
        "scene-1",
        () => ({ v: 1 }) as unknown as TiptapContent,
        0,
      );
      const save2 = saveCoordinator.scheduleSave(
        "scene-1",
        () => ({ v: 2 }) as unknown as TiptapContent,
        0,
      );
      const save3 = saveCoordinator.scheduleSave(
        "scene-1",
        () => ({ v: 3 }) as unknown as TiptapContent,
        0,
      );
  ...
  describe("Status Tracking", () => {
    it("isSaving MUST return true while save is in progress", async () => {
      ...
      const invokePromise = new Promise<void>((r) => {
        resolveInvoke = r;
      });
      ...
      const savePromise = saveCoordinator.scheduleSave(
        "waiting-scene",
        () => ({ type: "doc", content: [] }) as unknown as TiptapContent,
        0,
      );
  ...
  describe("Error Handling - MUST create backup on failure", () => {
    ...
    it("MUST handle deleted scene gracefully without error", async () => {
      vi.mocked(invoke).mockRejectedValue(
        new Error("Scene not found in structure"),
      );
  ...
  describe("Content Serialization", () => {
    it("MUST serialize content to JSON string for Tauri", async () => {
      ...
      await saveCoordinator.scheduleSave(
        "serialize-scene",
        () => complexContent as unknown as TiptapContent,
        5,
      );
    ...
    it("MUST handle non-serializable content gracefully (Promises, functions)", async () => {
      ...
      await saveCoordinator.scheduleSave(
        "weird-content",
        () => contentWithPromise as unknown as TiptapContent,
        0,
      );
