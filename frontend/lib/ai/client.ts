import { streamText, generateText as sdkGenerateText } from 'ai';
...
function getConnection(modelId: string): AIConnection {
    ...
    const connection = connections.find(c => c.enabled && c.models?.includes(modelId));
    ...
    if (!connection) {
        ...
        throw new Error(`No AI connection found for model: ${modelId}. Please configure a connection in Settings.`);
    }
 ...
 */
export function getConnectionForModel(modelId: string): AIConnection | undefined {
    const connections = storage.getItem<AIConnection[]>('ai_connections', []);
