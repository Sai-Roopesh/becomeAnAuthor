...
export class TauriCodexRelationRepository implements ICodexRelationRepository {
  async getByParent(
    ...
    try {
      ...
      throw toAppError(
        error,
        "E_CODEX_RELATION_LIST_FAILED",
        "Failed to load codex relations",
      );
    }
  }
  ...
  async create(
    seriesId: string,
    relation: Partial<CodexRelation> & { parentId: string; childId: string },
  ): Promise<CodexRelation> {
    ...
    try {
      ...
      throw toAppError(
        error,
        "E_CODEX_RELATION_SAVE_FAILED",
        "Failed to save codex relation",
      );
    }
  }
  ...
  async delete(seriesId: string, id: string): Promise<void> {
    ...
    try {
      ...
      throw toAppError(
        error,
        "E_CODEX_RELATION_DELETE_FAILED",
        "Failed to delete codex relation",
      );
    }
  }
}
