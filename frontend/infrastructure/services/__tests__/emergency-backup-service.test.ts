...
// Import after mocks
import { EmergencyBackupService, emergencyBackupService } from '@/infrastructure/services/emergency-backup-service';
...
describe('EmergencyBackupService Contract', () => {
    ...
    describe('Backup Creation', () => {
        it('MUST save backup with 24-hour expiry', async () => {
            ...
            expect(backup.expiresAt).toBeLessThanOrEqual(afterSave + 24 * 60 * 60 * 1000 + 1000);
        });
        ...
        it('MUST return true on successful save', async () => {
            ...
            const result = await service.saveBackup('scene-1', { type: 'doc', content: [] });
    ...
    describe('Backup Retrieval', () => {
        it('MUST return backup when it exists', async () => {
            ...
            expect(invoke).toHaveBeenCalledWith('get_emergency_backup', { sceneId: 'scene-1' });
        });
    ...
    describe('Backup Deletion', () => {
        ...
        it('MUST do nothing if backup does not exist', async () => {
            ...
            expect(invoke).toHaveBeenCalledWith('get_emergency_backup', expect.anything());
        });
