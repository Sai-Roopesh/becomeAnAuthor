...
describe('GoogleAuthService Contract', () => {
    ...
    describe('SPEC: signOut - Security Cleanup', () => {
        ...
        it('MUST clear all auth data from storage on sign out', async () => {
            ...
            // Implementation uses localStorage.removeItem directly
            // This tests actual behavior
            expect(localStorage.removeItem).toHaveBeenCalledWith(STORAGE_KEYS.GOOGLE_TOKENS);
            expect(localStorage.removeItem).toHaveBeenCalledWith(STORAGE_KEYS.GOOGLE_USER);
        });
    });
});
