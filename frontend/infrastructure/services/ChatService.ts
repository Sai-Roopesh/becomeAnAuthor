...
export class ChatService implements IChatService {
  ...
  async generateResponse(
    ...
    const context = params.context
      ? await this.buildContextText(params.context, params.projectId)
      : "";
    const history = await this.getConversationHistory(params.threadId);
    ...
    const result = await generate({
      model: params.model,
      messages,
      ...(params.settings.maxTokens && {
        maxTokens: params.settings.maxTokens,
      }),
      ...(params.settings.temperature != null && {
  ...
  async buildContextText(
    ...
    if (context.codexEntries?.length) {
      ...
      if (project?.seriesId) {
        const allEntries = await this.codexRepository.getBySeries(
          project.seriesId,
        );
