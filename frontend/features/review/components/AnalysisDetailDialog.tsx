...
export function AnalysisDetailDialog({
  ...
  const getSeverityBadge = (severity: number) => {
    ...
    const variant =
      severity === 2
        ? "outline"
        : variants[severity as keyof typeof variants] || "default";
    const className = severity === 2 ? "border-yellow-500 text-yellow-500" : "";

    return (
      <Badge
        variant={variant}
        className={cn("uppercase text-2xs font-bold tracking-wider", className)}
      >
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl h-[85vh] flex flex-col p-0 gap-0 bg-background/95 backdrop-blur-xl border-border/50 overflow-hidden">
        ...
        <Tabs defaultValue="overview" className="flex-1 flex flex-col min-h-0">
          ...
          <TabsContent
            value="overview"
            className="flex-1 min-h-0 mt-0 bg-muted/5"
          >
                                ...
                                <div className="space-y-2">
                                  ...
                                  {insight.autoSuggest && (
                                    <div className="bg-background/50 p-3 rounded-lg text-sm border border-destructive/10">
                                      <span className="font-semibold text-destructive text-xs uppercase tracking-wide block mb-1">
                                        Suggestion
                                      </span>
                                      {insight.autoSuggest}
                                    </div>
                          ...
                          <div className="flex-1 space-y-3">
                            ...
                            {insight.autoSuggest && (
                              <div className="bg-muted/30 p-4 rounded-lg text-sm border border-border/50">
                                <span className="font-semibold text-primary text-xs uppercase tracking-wide block mb-1">
                                  Suggestion
                                </span>
                                {insight.autoSuggest}
                              </div>
                            ...
                            {insight.sceneReferences &&
                              insight.sceneReferences.length > 0 && (
                                <div className="flex flex-wrap gap-2 pt-2">
                                  {insight.sceneReferences.map((ref, idx) => (
                                    <Badge
                                      key={idx}
                                      variant="outline"
                                      className="text-2xs bg-background"
                                    >
                                      {ref.sceneTitle}
                                    </Badge>
                                  ))}
                    ...
                    <div className="grid grid-cols-2 gap-6 text-sm">
                      <div className="space-y-1.5">
                        ...
                        <span className="font-medium text-lg">
                          {analysis.wordCountAtAnalysis.toLocaleString()}
                        </span>
                      </div>
                      <div className="space-y-1.5">
                        <span className="text-muted-foreground text-xs uppercase tracking-wide block">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <span className="font-medium text-lg">
                          {analysis.tokensUsed?.toLocaleString() || "Unknown"}
                        </span>
                      </div>
                    </div>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                ...
                {analysis.analysisType.replace("-", " ")} analysis? This action
                cannot be undone.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
            ...
            <div className="flex flex-wrap gap-2">
              {data["themes"].map((theme: string, idx: number) => (
                <Badge
                  key={idx}
                  variant="secondary"
                  className="px-4 py-1.5 text-sm rounded-full bg-secondary/50 border border-border/50"
                >
                  {theme}
                </Badge>
              ))}
            ...
            <CardHeader className="pb-3">
              <CardTitle className="text-lg font-heading flex items-center justify-between">
                ...
                <Badge
                  ...
                  className="uppercase text-2xs tracking-wider"
                >
                  {thread.status}
                </Badge>
              </CardTitle>
  ...
  return (
    <Card>
      <CardContent className="p-4">
        <pre className="text-xs bg-muted p-4 rounded-lg overflow-auto max-h-modal">
