...
export function ManuscriptTreeSelector({
  projectId,
  selected,
  onSelect,
}: ManuscriptTreeSelectorProps) {
    ...
    return (
      <div key={node.id}>
        <div
          ...
          style={{
            paddingLeft: `${level * TREE_INDENT_PX + TREE_BASE_PADDING_PX}px`,
          }}
        >
          {hasChildren && (
            <button
          ...
          <Checkbox
            ...
            onCheckedChange={(checked) =>
              toggleSelected(node.id, checked as boolean)
            }
          />

          <Icon className="h-4 w-4 text-muted-foreground" />

          <label htmlFor={node.id} className="flex-1 text-sm cursor-pointer">
            {node.title}
          </label>
        </div>
