...
const QUICK_SUGGESTIONS = [
  {
    icon: Lightbulb,
    text: "Help me brainstorm character ideas",
    color: "text-amber-500",
  },
  { icon: BookOpen, text: "Outline my next chapter", color: "text-blue-500" },
  {
    icon: Pencil,
    text: "Describe this scene's setting",
    color: "text-green-500",
  },
  {
    icon: Users,
    text: "Develop my protagonist's backstory",
...
export function ChatMessageList({
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground p-8">
        ...
        <p className="text-sm max-w-xs text-center mb-6">
          Start a conversation to brainstorm ideas, outline your story, or get
          writing assistance.
        </p>
        ...
        {onSuggestionClick && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-w-lg w-full px-4">
            {QUICK_SUGGESTIONS.map((suggestion, idx) => (
              ...
              >
                <suggestion.icon
                  className={`h-4 w-4 mr-2.5 flex-shrink-0 ${suggestion.color}`}
                />
  ...
  return (
    <div className="flex-1 overflow-y-auto min-h-0 p-4 space-y-6 pb-32 scroll-smooth">
      {messages.map((msg) => (
