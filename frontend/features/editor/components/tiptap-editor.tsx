...
import { SlashCommands } from "@/lib/tiptap-extensions/slash-commands";
...
export function TiptapEditor({
  ...
  const [customRoomId, setCustomRoomId] = useState<string | undefined>(
    undefined,
  );
  const {
    ydoc,
      ...
      if (previousSceneIdRef.current !== sceneId) {
        ...
        if (
          ...
          try {
            ...
            log.error("Failed to save scene before switch:", error);
          }
        }
            ...
            try {
              ...
              parsedContent = {
                ...
                content: [
                  {
                    type: "paragraph",
                    content: [{ type: "text", text: scene.content }],
                  },
                ],
              };
            }

            log.debug(`Loaded FRESH content from file`, {
