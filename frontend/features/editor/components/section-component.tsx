...
export function SectionComponent({
  ...
  const { title, color, sectionType, excludeFromAI, collapsed } = attrs;
      ...
      >
        ...
        <div className="bg-muted/30 p-2 flex items-center gap-2">
          ...
          {isEditing ? (
            ...
            >
              {title || DEFAULT_SCENE_SECTION_TITLE}
            </button>
          )}

          {sectionType !== "standard" && (
            <div className="text-[11px] bg-primary/10 text-primary px-2 py-0.5 rounded">
              {SECTION_TYPES.find((item) => item.value === sectionType)
                ?.label || "Standard"}
            </div>
          )}
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem
                onClick={() =>
                  updateAttributes({ excludeFromAI: !excludeFromAI })
                }
              >
                ...
                <DropdownMenuSubContent>
                  {SECTION_TYPES.map((type) => (
                    <DropdownMenuItem
                      key={type.value}
                      onClick={() =>
                        updateAttributes({ sectionType: type.value })
                      }
                    >
                      {type.label}
                    </DropdownMenuItem>
