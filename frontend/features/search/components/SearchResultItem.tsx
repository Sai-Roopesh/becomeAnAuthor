...
export function SearchResultItem({ result, type, isSelected, onClick }: SearchResultItemProps) {
        ...
        >
            ...
            <div className="flex-1 min-w-0">
                <div className="font-medium text-sm truncate">
                    {type === 'scene' ? (result.item as SearchableScene).title : (result.item as SearchableCodex).name}
                </div>
                <div className="text-xs text-muted-foreground truncate">
                    {type === 'scene'
                        ? (result.item as SearchableScene).summary || 'No summary'
                        : (result.item as SearchableCodex).category}
                </div>
            </div>
            {isSelected && (
