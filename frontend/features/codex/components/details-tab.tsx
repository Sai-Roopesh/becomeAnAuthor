...
import {
  codexAttributeSchema,
  type CodexAttributeFormData,
...
export function DetailsTab({ entity, onChange }: DetailsTabProps) {
  const {
    ...
    reset,
    formState: { isValid },
  } = useForm<CodexAttributeFormData>({
    resolver: zodResolver(codexAttributeSchema),
    mode: "onChange",
  });
...
Generate now for: ${entity.name} (${entity.category})`;
  ...
  const addAttribute = (data: CodexAttributeFormData) => {
    const newAttributes = { ...entity.attributes, [data.key]: data.value };
  ...
  const wordCount =
    entity.description?.split(/\s+/).filter(Boolean).length || 0;
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="space-y-2">
        ...
        <div className="border rounded-lg p-4 space-y-4">
          ...
          <div className="space-y-2">
            {Object.entries(entity.attributes || {}).map(([key, value]) => (
              <div key={key} className="flex items-center gap-2">
                <div className="w-1/3 text-sm font-medium bg-muted/50 px-2 py-1 rounded">
                  {key}
                </div>
          ...
          {/* Add New */}
          <form
            onSubmit={handleSubmit(addAttribute)}
