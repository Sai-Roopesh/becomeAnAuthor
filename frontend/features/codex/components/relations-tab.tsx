...
import { Plus, X, Link2, User, MapPin, Box, Book, FileText, Search } from 'lucide-react';
        ...
        <div className="max-w-3xl space-y-4">
            ...
            {relatedEntries && relatedEntries.length > 0 ? (
                <div className="space-y-2">
                    <h4 className="text-sm font-medium text-muted-foreground">Linked entries</h4>
                        ...
                        return (
                            <div key={entry.id} className="flex items-center gap-3 p-3 border rounded hover:bg-accent">
                                <div className="h-8 w-8 bg-muted rounded flex items-center justify-center flex-shrink-0">
                                    {categoryIcons[entry.category] || <Link2 className="h-4 w-4 text-muted-foreground" />}
                                </div>
                        ...
                        {availableEntries.length > 0 ? (
                            <div className="space-y-1">
                                {availableEntries.map(entry => (
                                    ...
                                    >
                                        <div className="h-8 w-8 bg-muted rounded flex items-center justify-center flex-shrink-0">
                                            {categoryIcons[entry.category] || <Link2 className="h-4 w-4" />}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <div className="font-medium text-sm">{entry.name}</div>
                                            <div className="text-xs text-muted-foreground capitalize">{entry.category}</div>
                            ...
                            <p className="text-sm text-muted-foreground text-center py-8">
                                {searchQuery ? 'No matching entries' : 'No available entries to link'}
                            </p>
                        )}
                    </ScrollArea>
                </DialogContent>
