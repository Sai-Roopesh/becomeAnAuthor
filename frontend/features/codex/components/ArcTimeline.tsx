...
import { CHART_COLORS_HEX } from "@/shared/constants/colors";

interface ArcTimelineProps {
  entry: CodexEntry;
  seriesId: string;
  onAddPoint: (
    point: Omit<ArcPoint, "id" | "createdAt" | "updatedAt">,
  ) => Promise<void>;
...
export function ArcTimeline({
  ...
  const arcPoints = [...(entry.arcPoints || [])].sort(
  ...
  // Colors for graph lines - use centralized constants
  const colors = CHART_COLORS_HEX.light;
      ...
      <div className="space-y-3">
        ...
        {arcPoints.map((point) => (
          ...
          >
            ...
            {/* Content */}
            <div
              className="bg-card p-3 rounded-lg border hover:border-primary/50 transition-colors cursor-pointer"
              onClick={() => {
            ...
            >
              ...
              {point.knowledgeState?.knows &&
                point.knowledgeState.knows.length > 0 && (
                  <div className="text-xs mt-2 p-2 bg-blue-50 dark:bg-blue-950/20 rounded">
                    <strong className="text-blue-700 dark:text-blue-400">
                      Knows:
                    </strong>{" "}
                    {point.knowledgeState.knows.join(", ")}
                  </div>
                )}
              {point.emotionalState?.primaryEmotion && (
                <div className="text-xs mt-2 p-2 bg-red-50 dark:bg-red-950/20 rounded">
                  ...
                  {point.emotionalState.primaryEmotion}
                  {point.emotionalState.intensity &&
                    ` (${point.emotionalState.intensity}/10)`}
                </div>
              )}
