...
interface OutlineViewProps {
  projectId: string;
  seriesId: string; // Required - series-first architecture (for consistency)
  nodes: DocumentNode[];
}

export function OutlineView({ projectId, nodes }: OutlineViewProps) {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 pb-20">
      {acts.map((act, actIndex) => {
        ...
        const isActExpanded =
          expandedNodes.has(act.id) || expandedNodes.size === 0;
        ...
        return (
          ...
          >
            ...
            <div className="sticky top-0 z-10 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-y border-border/50 py-3 px-4 flex items-center gap-3 shadow-sm mb-4">
              ...
              <h2 className="text-xl font-heading font-bold flex-1 tracking-tight">
                {act.title}
              </h2>
              ...
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  openCreateDialog(act.id, "chapter");
                }}
              >
              ...
              <div className="pl-4 pr-2 space-y-6">
                {chapters.map((chapter) => {
                  ...
                  const isChapterExpanded =
                    expandedNodes.has(chapter.id) || expandedNodes.size === 0;

                  return (
                    <div
                      key={chapter.id}
                      className="relative pl-6 border-l-2 border-border/30 hover:border-primary/30 transition-colors"
                    >
                      {/* Chapter Header */}
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0 space-y-1">
                                <div className="flex items-center justify-between gap-2">
                                  <h4 className="font-medium text-base text-foreground group-hover/scene:text-primary transition-colors">
                                    {scene.title}
                                  </h4>
