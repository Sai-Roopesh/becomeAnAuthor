...
export function TimelineView({
  ...
  const [visibleCategories, setVisibleCategories] = useState<LaneCategory[]>([
    "character",
    "subplot",
    "location",
    "item",
  ...
  const visibleLanes = useMemo(() => {
    ...
    return codexEntries
      .filter((entry) =>
        visibleCategories.includes(entry.category as LaneCategory),
      )
    ...
    return filteredScenes
      .map((scene, index) => {
        const hasLink = links.some(
          (l) => l.sceneId === scene.id && l.codexId === codexId,
        );
          ...
          <div className="relative min-w-max">
            <div className="flex">
              ...
              <div className="flex flex-col">
                {/* Top Scene Track */}
                <div
                  className="h-10 border-b bg-muted/30 flex items-center px-4 sticky top-0 z-10 grid gap-2"
                  style={{
                    gridTemplateColumns: `repeat(${filteredScenes.length}, minmax(120px, 1fr))`,
                  }}
                >
                  {filteredScenes.map((scene, i) => (
                    <div
                      key={scene.id}
                      className="text-xs font-medium text-muted-foreground truncate px-1 border-l first:border-l-0 border-border/50"
                    >
                      {i + 1}. {scene.title}
                    </div>
                  ))}
