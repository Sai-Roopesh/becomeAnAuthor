                    ...
                    >
                        ...
                        <div className="flex items-center gap-4 mb-4">
                            ...
                            <div className="flex items-center gap-3 bg-card border border-border/50 shadow-sm px-4 py-2 rounded-full">
                                ...
                                <h2 className="text-lg font-heading font-bold text-foreground tracking-tight">{act.title}</h2>
                        ...
                        {isExpanded && (
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 px-4">
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="p-4 border-b border-border/50 bg-muted/20 flex items-center justify-between group-hover/chapter:bg-muted/40 transition-colors">
                                                <h3 className="font-heading font-semibold text-foreground truncate" title={chapter.title}>
                                                    {chapter.title}
                                                </h3>
                                                <Button
                                                    variant="ghost"
                                            ...
                                            <div className="p-3 space-y-2 flex-1 min-h-[100px]">
                                                ...
                                                {scenes.length === 0 && (
                                                    <div className="h-full flex flex-col items-center justify-center text-muted-foreground/50 py-8 border-2 border-dashed border-border/30 rounded-lg m-1">
                                                        <span className="text-xs font-medium">Empty Chapter</span>
                                                        <Button
                                                            variant="link"
                                                            size="sm"
                                                            className="h-auto p-0 text-[10px] mt-1 text-primary/70"
                                                            onClick={() => openCreateDialog(chapter.id, 'scene')}
                                                        >
                                                            + Add Scene
                                                        </Button>
                                            ...
                                            {/* Footer Stats */}
                                            <div className="px-4 py-2 bg-muted/10 border-t border-border/30 text-[10px] text-muted-foreground flex justify-between">
                                                <span>{scenes.length} Scenes</span>
                                                <span>{scenes.reduce((acc, s) => acc + (s.type === 'scene' ? (s.wordCount || 0) : 0), 0)} words</span>
                                            </div>
                                        </div>
                                    );
                                })}
                                ...
                                >
                                    ...
                                    <span className="text-sm font-medium text-muted-foreground group-hover/add:text-primary">New Chapter</span>
                                </button>
                            </div>
                        )}
                    </div>
