...
export function SceneCard({ scene, seriesId }: SceneCardProps) {
  ...
  const wordCount =
    scene.type === "scene" && "wordCount" in scene ? scene.wordCount : 0;
  const linkedCount = sceneLinks?.length ?? 0;
  const linkActionLabel =
    linkedCount > 0 ? `${linkedCount} linked` : "Link Codex";

  return (
    <div
      className="border rounded-lg p-3 hover:bg-accent/50 transition-colors group cursor-pointer"
      onClick={openScene}
    >
      {/* Codex Badges Row */}
      <div
