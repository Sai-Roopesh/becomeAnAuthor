...
export function SceneCodexBadges({
  ...
  const { sceneCodexLinkRepository: linkRepo, codexRepository: codexRepo } =
    useAppServices();
  ...
  const groupedByCategory = entries.reduce(
    (acc, entry) => {
  ...
  const sortedEntries = categoryOrder.flatMap(
  ...
  const sizeClasses = size === "sm" ? "h-5 w-5 text-2xs" : "h-6 w-6 text-xs";
          ...
          <Tooltip key={entry.id}>
            ...
            <TooltipContent side="top" className="text-xs">
              <div className="flex items-center gap-1.5">
                ...
                {isPov && (
                  <span className="text-2xs px-1 py-0.5 bg-primary/10 text-primary rounded">
                    POV
                  </span>
                )}
              </div>
          ...
          <TooltipTrigger asChild>
            <div
              className={`
                                ...
                                rounded-full flex items-center justify-center
                                text-2xs font-medium cursor-pointer
