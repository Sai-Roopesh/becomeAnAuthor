...
export function SnippetEditor({
  ...
  const [initialContent, setInitialContent] = useState<TiptapContent | null>(
    null,
  );
  ...
  const editor = useEditor(
    {
      ...
      editorProps: {
        attributes: {
          class:
            "prose prose-sm dark:prose-invert max-w-none focus:outline-none h-full p-4",
        },
      },
      immediatelyRender: false,
    },
    [initialContent],
  ); // Re-create editor when initialContent changes (loaded)
      ...
      <div className="flex items-center gap-2 p-2 border-b">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="icon" aria-label="Snippet actions">
