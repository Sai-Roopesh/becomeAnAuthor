...
const log = logger.scope("ModelSelector");
import {
  Select,
  SelectContent,
  SelectItem,
...
/**
 * Get icon for provider - using VendorLogo component
 */
function ProviderIcon({
  ...
  return <VendorLogo providerId={provider as AIProvider} size={size} />;
}
...
export function ModelSelector({
  ...
  const handleRefresh = useCallback(async () => {
    ...
    try {
      ...
      // User feedback on refresh failure
        ...
        <SelectTrigger className={cn("flex-1", className)}>
          <SelectValue placeholder="Select a model">
            {value && (
              <div className="flex items-center gap-2">
                {currentProvider && (
                  <ProviderIcon provider={currentProvider} size={16} />
                )}
              ...
              Array.from(groupedModels.entries()).map(
                ([provider, providerModels]) => (
                  <SelectGroup key={provider}>
                    <SelectLabel className="sticky top-0 z-10 bg-popover py-1.5 text-xs font-semibold text-muted-foreground flex items-center gap-1.5">
