    ...
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center">
        ...
        <p className="text-muted-foreground mb-6 max-w-sm">
          Create a series to organize related books together, like trilogies or
          multi-book sagas.
        </p>
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            {series.length} series • {projects.filter((p) => p.seriesId).length}{" "}
            books in series • Click a series to open
          </p>
        </div>
        <Button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {series.map((s) => {
          ...
          return <SeriesCard key={s.id} series={s} projects={seriesProjects} />;
        })}
      </div>
