...
export function ContextSelector({
  ...
  const { nodeRepository: nodeRepo, codexRepository: codexRepo } =
    useAppServices();
  ...
  const codexByCategory = (codexEntries ?? []).reduce(
    (acc, entry) => {
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      ...
      case "lore":
        return <Scroll className="h-4 w-4 mr-2" />;
      default:
  ...
  return (
    <div className="space-y-2">
      {selectedContexts.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {selectedContexts.map((context, index) => (
            ...
            >
              <span className="truncate max-w-select-sm">{context.label}</span>
              <button
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent
          align="start"
          className="w-64 max-h-modal overflow-y-auto"
        >
          {/* Global Contexts */}
          <DropdownMenuItem
            onClick={() =>
              addContext({ type: "novel", label: "Full Novel Text" })
            }
          >
          ...
          <DropdownMenuItem
            onClick={() =>
              addContext({ type: "outline", label: "Full Outline" })
            }
          >
          ...
          <DropdownMenuSub>
            ...
            <DropdownMenuSubContent className="w-56 max-h-dropdown overflow-y-auto">
              ...
              {acts.length === 0 ? (
                ...
                acts.map((act) => (
                  <DropdownMenuItem
                    key={act.id}
                    onClick={() =>
                      addContext({ type: "act", id: act.id, label: act.title })
                    }
                  >
                    {act.title}
                  </DropdownMenuItem>
          ...
          <DropdownMenuSub>
            ...
            <DropdownMenuSubContent className="w-64 max-h-dropdown overflow-y-auto">
